            ...
            if (lockDb) {
                ...
                if (nRefCount == 0) {
                    ...
                    if (mi != bitdb.mapFileUseCount.end()) {
                        LogPrint("db", "Flushing makerbux.dat\n");
                        nLastFlushed = nWalletDBUpdated;
                        int64_t nStart = GetTimeMillis();

                        // Flush makerbux.dat so it's self contained
                        bitdb.CloseDb(strFile);
                        bitdb.CheckpointLSN(strFile);

                        bitdb.mapFileUseCount.erase(mi++);
                        LogPrint("db", "Flushed makerbux.dat %dms\n", GetTimeMillis() - nStart);
                    }
                }
            }
        }
    ...
    while (true) {
        {
            ...
            if (!bitdb.mapFileUseCount.count(wallet.strWalletFile) || bitdb.mapFileUseCount[wallet.strWalletFile] == 0) {
                ...
                try {
                    ...
                    dst << src.rdbuf();
#endif
                    LogPrintf("copied makerbux.dat to %s\n", pathDest.string());
                    return true;
                } catch (const filesystem::filesystem_error& e) {
                    LogPrintf("error copying makerbux.dat to %s - %s\n", pathDest.string(), e.what());
                    return false;
                }
            }
        }
...
//
// Try to (very carefully!) recover makerbux.dat if there is a problem.
//
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    // Recovery procedure:
    // move makerbux.dat to wallet.timestamp.bak
    // Call Salvage with fAggressive=true to
    // get as much data as possible.
    // Rewrite salvaged data to makerbux.dat
