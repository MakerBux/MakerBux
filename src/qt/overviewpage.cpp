...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MKRBX)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MKRBX")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeMakerBuxAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMakerBuxAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeMakerBuxAmount = strAnonymizeMakerBuxAmount.remove(strAnonymizeMakerBuxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMakerBuxAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeMakerBuxAmount * COIN) nMaxToAnonymize = nAnonymizeMakerBuxAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeMakerBuxAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeMakerBuxAmount));
        strAnonymizeMakerBuxAmount = strAnonymizeMakerBuxAmount.remove(strAnonymizeMakerBuxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMakerBuxAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
